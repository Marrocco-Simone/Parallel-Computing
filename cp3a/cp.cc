...
#pragma omp parallel for
  for (int j = 0; j < ny; j++)
  {
    for (int i = j; i < ny; i++)
    {
      ...
      while (k < nnx - 4)
      {
        sum1 += normalized[k + 0 + i * nnx] * normalized[k + 0 + j * nnx];
        sum2 += normalized[k + 1 + i * nnx] * normalized[k + 1 + j * nnx];
        sum3 += normalized[k + 2 + i * nnx] * normalized[k + 2 + j * nnx];
        sum4 += normalized[k + 3 + i * nnx] * normalized[k + 3 + j * nnx];
        k += 4;
      }
      while (k < nnx)
      {
      ...
      result[i + j * ny] = cumsum1 + cumsum2 + cumsum3 + cumsum4;
    }
  }
}
